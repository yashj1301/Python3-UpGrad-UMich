...
where 
    (m.date_published between '2017-03-01' and '2017-03-31') 
                        and  
    m.country rlike'USA' and r.total_votes>1000
...
from 
    ...
    from 
        (select avg(total_votes) 'Germany_Votes' from movie,ratings
        where movie.id=ratings.movie_id and languages rlike 'German') as tb1,
        (select avg(total_votes) 'Italy_Votes' from movie,ratings
        where movie.id=ratings.movie_id and languages rlike 'Italian') as tb2
    ) 
as GERvsITA;
...
(select count(*) 'known_for_movies_nulls' from names where known_for_movies is NULL) as kfm_null;
...
select * from 
    ...
    rank() over (partition by genre,year order by worldwide_gross_income desc) 'movie_rank' from 
        (select genre, year,title 'movie_name',
        sum(cast(trim( '$ ' from worlwide_gross_income) as signed)) 'worldwide_gross_income'
        from genre g inner join movie m on g.movie_id=m.id
        where genre in 
...
select * from 
    (select name,sum(total_votes) 'total_votes',count(m.id) 'num_movies', 
    round(sum(avg_rating*total_votes)/sum(total_votes),2) 'actress_avg_rating',
    dense_rank() over 
    (order by sum(avg_rating*total_votes)/sum(total_votes) desc, 
          sum(total_votes) desc) 'actress_rank'
    from movie m
    ...
    group by name having actress_avg_rating>8 
    order by actress_avg_rating desc)
as f_rank where actress_rank <=3;
...
as dir_summary
group by director_id order by number_of_movies desc,avg_rating desc limit 9;
